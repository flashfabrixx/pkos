...
PKOS should become useful as an open-source project before its architecture is fully mature. That means code and documentation need to be understandable to outside readers, not only to the original operator.
...
PKOS will keep the v1 implementation simple, but the project structure should preserve clear public boundaries:
...
Agentic integrations should talk to PKOS through stable HTTP APIs first. Direct database access should be reserved for local development and migrations.
