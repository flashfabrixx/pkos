...
PKOS ships two complementary backup paths. Pick the one that matches
your operational style.

## A. Portable JSONL archive — `pkos:export`
...
application's `pkos.v1` shape.

```bash
pnpm pkos:export --out backup-$(date +%F).tar.gz
# optional: include rows currently in the trash
pnpm pkos:export --out backup.tar.gz --include-trashed
```
...
```bash
DATABASE_URL=postgres://… pnpm pkos:import --in backup.tar.gz
# preview without writing
DATABASE_URL=postgres://… pnpm pkos:import --in backup.tar.gz --dry-run
```
...
```bash
pg_dump --format=custom --file=pkos.pgdump "$DATABASE_URL"
# restore:
createdb pkos
pg_restore --dbname=pkos --no-owner --no-privileges pkos.pgdump
```

Don't forget to back up the **files** directory (`PKOS_FILES_PATH`)
alongside the database — attachments live there, not inside Postgres.

## C. Markdown vault

The vault directory (`PKOS_VAULT_PATH`) already contains one Markdown
...
```bash
rsync -a "$PKOS_VAULT_PATH"/ /your/backup/dest/
```
