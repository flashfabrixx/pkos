  ...
  const [documents, actions, related, comments, stats] = await Promise.all([
    ...
    query(
      `SELECT
         ...
         (SELECT COUNT(*)::int FROM action_items WHERE project_id = $1 AND status = 'open') AS actions_open,
         (SELECT COUNT(*)::int FROM action_items WHERE project_id = $1 AND status = 'done') AS actions_done,
