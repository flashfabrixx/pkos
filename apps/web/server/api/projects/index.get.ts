...
export default defineEventHandler(async (event) => {
  ...
  let whereClause = `e.type = 'project' AND e.deleted_at IS NULL`
  if (q) {
    values.push(`%${q}%`)
  ...
  const result = await query<{
    ...
    `SELECT e.id, e.name,
       ...
       (SELECT COUNT(*)::int FROM action_items WHERE project_id = e.id AND status = 'open' AND deleted_at IS NULL) AS actions_open,
       (SELECT COUNT(*)::int FROM action_items WHERE project_id = e.id AND status = 'done' AND deleted_at IS NULL) AS actions_done,
