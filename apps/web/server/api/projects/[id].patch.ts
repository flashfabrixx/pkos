...
export default defineEventHandler(async (event) => {
  ...
  const beforeResult = await query<{ name: string, description: string | null }>(
    `SELECT name, (metadata->>'description') AS description FROM entities WHERE id = $1 AND type = 'project' AND deleted_at IS NULL`,
    [id]
  )
  ...
  try {
    const result = await query(
      `UPDATE entities
       ...
       WHERE id = $${values.length} AND type = 'project' AND deleted_at IS NULL
       RETURNING id, name, canonical_name, metadata`,
      values
    )
    if (!result.rowCount) throw createError({ statusCode: 404, statusMessage: 'Project not found' })
    ...
    if (error?.code === '23505') {
      throw createError({ statusCode: 409, statusMessage: 'Another project already uses this name' })
    }
    throw error
  }
})
